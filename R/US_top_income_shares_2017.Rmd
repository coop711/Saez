...
plot(P99_100 ~ Year, data = US_top_income_shares_17, xlab = "연도", ylab = "소득점유(%)", ylim = c(5, 25), xaxt = "n", type = "b", pch = 17)
```

이제 x-축에는 연도를 10년 단위로 표시하고, `lines()`를 이용하여 차상위4%와 차차상위5%의 소득 점유율 변화를 함께 그린다.

```{r, 95-99 and 90-95, fig.width = 12, fig.height = 6.75}
plot(P99_100 ~ Year, data = US_top_income_shares_17, xlab = "연도", ylab = "소득점유(%)", xlim = c(1910, 2020), ylim = c(5, 25), xaxt = "n", type = "b", pch = 17)
...
abline(h = seq(5, 25, by = 5), lty = 3)
abline(v = seq(1910, 2020, by = 10), lty = 3)
legend_text <- c("99-100%:$442,900 이상(2017년 기준)", "95-99%:$180,500-$442,900", "90-95%:$124,810-$180,500")
...
main_title <- "미국 소득 상위 10%의 점유율 분할"
title(main = main_title)
y_1928 <- US_top_income_shares_17$P99_100[US_top_income_shares_17$Year == 1928]
y_2007 <- US_top_income_shares_17$P99_100[US_top_income_shares_17$Year == 2007]
...
```{r, reshape}
library(reshape2)
data_1_10 <- US_top_income_shares_17[c("Year", "P99_100", "P95_99", "P90_95")]
...
(g13 <- g12 + 
  annotate("text", x = c(1935, 1960, 2014), y = c(22, 8, 18), label = times_label, colour = "red", family = "HCR Dotum LVT", size = 8))
```

---
title: "US Top Income Share 2017"
author: "coop711"
date: "`r Sys.Date()`"
...
준비한 자료는  [E. Saez 교수의 홈페이지](http://elsa.berkeley.edu/~saez/)에 있는 `TabFig2017prel.xls` 를 손봐서 불러들인 것이다. 

```{r, data preparation, message = FALSE, echo = FALSE, results = 'hide'}
options(digits = 2)
# library(xlsx)
load("US_top_income_shares_2017_add.rda")
# US_top_income_shares_17 <- read.xlsx("../data/TabFig2017prel.xlsx", sheetIndex = 9, sheetName = "Table A3", startRow = 6, endRow = 107, colIndex = c(1:7, 9:13), header = FALSE)
# v_names <- read.xlsx("./data/TabFig2017prel.xlsx", sheetName = "Table A3", startRow = 4, endRow = 4, colIndex = c(2:7, 9:17), colClasses = character, header = FALSE)
...
# plot(x = US_top_income_shares_17$Year, y = US_top_income_shares_17$P99_100)
...
`TabFig2017prel.xls`의 `Table 0` 시트에 나와 있는 정보를 활용하여 범례를 만든다.
...
abline(h = seq(5, 25, by = 5), lty = 3)
abline(v = seq(1910, 2020, by = 10), lty = 3)
legend.text <- c("99-100%: More than $442,900 (2017)", "95-99%:$180,500-$442,900", "90-95%:$124,810-$180,500")
legend(x = 1947, y = 25, legend = legend.text, pch = 17, col = c("black", "red", "blue"))
```
...
abline(v = seq(1910, 2020, by = 10), lty = 3)
legend(x = 1947, y = 25, legend = legend.text, pch = 17, col = c("black", "red", "blue"))
main.title <- "US Top 10% Income Shares Decomposition"
...
legend(x = 1947, y = 25, legend = legend.text, pch = 17, col = c("black", "red", "blue"))
title(main = main.title, cex.main = 2)
y.1928 <- US_top_income_shares_17$P99_100[US_top_income_shares_17$Year == 1928]
y.2007 <- US_top_income_shares_17$P99_100[US_top_income_shares_17$Year == 2007]
...
text(x = c(1937, 1960, 2017), y = c(22, 8, 18.5), label = times.label, cex = 1.5, col = "red")
```
...
leg.title <- "Income Fractiles"
(g6 <- g5 + 
   labs(colour = leg.title, fill = leg.title) )
```
...
(g12 <- g11 + 
#   geom_text(data = data.frame(x = c(1928, 2007), y = c(y.1928, y.2007)), aes(x = x, y = y)))
  annotate("text", x = c(1928, 2007), y = c(y.1928, y.2007) + 0.7, label = c(1928, 2007)))
```
...
(g13 <- g12 + 
  annotate("text", x = c(1935, 1960, 2014), y = c(22, 8, 18), label = times.label, colour = "red", size = 5))
```

---
title: "US Top Income Share 2014 : Top 1%"
author: "coop711"
date: "`r Sys.Date()`"
...
```{r, top 0.1 percent scatter,  fig.width = 12, fig.height = 6.75}
...
이제 x-축에는 연도를 10년 단위로 표시하고, `lines()`를 이용하여 차상위 0.4%와 차차상위 0.5%의 소득 점유율 변화를 함께 그린다.

```{r, 99.5-99.9 and 99.0-99.5, fig.width = 12, fig.height = 6.75}
...
abline(h = seq(0, 15, by = 5), lty = 2)
abline(v = seq(1910, 2010, by = 10), lty = 2)
legend.text <- c("99.9-100%:$1,894,100 이상(2014년 기준)","99.5-99.9%:$646,550-$1,894,100","99-99.5%:$423,090-$646,550")
...
legend.text <- c("99.9-100%:$1,894,100 이상(2014년 기준)","99.5-99.9%:$646,550-$1,894,100","99-99.5%:$423,090-$646,550")
legend(x = 1945, y = 25, legend = legend.text, pch = 17, col = c("black", "red", "blue"))
main.title <- "미국 소득 상위 1%의 점유율 분할"
...
library(ggplot2)
(g0 <- ggplot(data.1.melt, aes(x = Year, y = Share, colour = Percentiles)) + 

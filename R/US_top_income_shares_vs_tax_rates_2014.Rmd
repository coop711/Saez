...
names(tax.rates) <- c("Year", "Min", "Max")
```
...
library(knitr)
top.income_tax <- cbind(US.top.income.shares.14[c("Year", "P99_100")], tax.rates[-1, 2:3])
...
par(new = TRUE)
plot(Max ~ Year, data = top.income_tax, type ="l", lty = 2, axes = FALSE, ann = FALSE, ylim = c(0, 100))
...
```{r, reshape}
library(reshape2)
top.income_tax.melt <- melt(top.income_tax, id.vars = "Year", measure.vars = c("P99_100", "Min", "Max"), variable.name = "Income_vs_Tax", value.name = "Percentage")
